...
RUN dotnet build ./src/Flash.Central.AdminApi/Flash.Central.AdminApi.csproj -c Release -o /build
...
ENTRYPOINT [ "/bin/bash" ]
CMD ["/app/admin-start.sh"]

COPY ./deploy/admin-start.sh /app/
